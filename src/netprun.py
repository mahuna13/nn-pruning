...
class Network(object):
    ...
    def __init__(self, layers, mini_batch_size):
        ...
        self.masks = [mask for layer in self.layers for mask in layer.masks]
        self.x = T.matrix("x")
        self.y = T.ivector("y")
    ...
    def apply_masks(self):
        apply = theano.function(
            [], [], 
            updates=[(param, param * mask) for param, mask in zip(self.params, self.masks)])
        apply()
...
class SoftmaxLayer(object):
    ...
    def __init__(self, n_in, n_out, p_dropout=0.0):
        ...
        self.w_mask = theano.shared(
            ...
            name='wm', borrow=True)
        self.b_mask = theano.shared(
            np.ones((n_out,), dtype=theano.config.floatX),
            name='bm', borrow=True)
        self.masks = [self.w_mask, self.b_mask]
